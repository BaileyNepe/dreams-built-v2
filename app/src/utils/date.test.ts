...
import { generateWeeks, getOrdinal } from './date'
...
describe('generateWeeks', () => {
  test.each([
    ...
    'generateWeeks returns correct periods',
    (startWeek, endWeek, numberOfPeriods, expected) => {
      expect(generateWeeks(startWeek, endWeek, numberOfPeriods)).toStrictEqual(
        expected,
      )
    },
  )
