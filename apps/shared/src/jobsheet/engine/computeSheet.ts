...
export const computeWall = (
  ...
  if (effective > 0 && blkSpans.length > 0) {
    ...
    blkSpans.forEach((span) => {
      ...
      // Inset run along the frame line. The entering BLK sits inside the
      // channel and consumes one board thickness; the outset BLK stands
      // above the shutter at the step, so the run goes all the way to the
      // outset line. A span reaching a wall end is open at the corner
      // instead, so the usual corner rules apply there (absorb, overhang,
      // cap). Polystyrene only where the short is trapped: a channel
      // enclosed between two BLKs, or a both-ends-internal wall.
      ...
      const inset = packRun(
        Math.max(0, insetEnd - insetStart + (trailingBlk ? 0 : capExtra)),
        {
          ...
          polystyreneShort:
            wall.polystyreneOverride ?? (polystyrene || (leadingBlk && trailingBlk)),
